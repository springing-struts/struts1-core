...
public class Field {
  ...
  public String getRequiredVarValue(String varName) {
    var aVar = varsByName.get(varName);
    if (aVar == null) throw new IllegalArgumentException(format(
  ...
  public String getMessageFor(
    ...
    var messageResources = ModuleUtils.getCurrent().getMessageResources(bundle);
    Object[] args = getArgs(validatorAction.getName()).stream().map(Arg::getText).toArray();
    var message = messageResources.getMessage(
      validatorAction.getMsg(), args
    );
    return requireNonNull(
      message,
      () -> format(
        "The validation message [%s] is not defined.",
