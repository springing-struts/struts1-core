...
public class ActionMessages {
  ...
  public void addValidationError(Field field, ValidatorAction validatorAction) {
    var args = field.getArgs(validatorAction.getName());
    var message = new ActionMessage(
      validatorAction.getMsg(),
      args.stream().filter(
        it -> it.getName().isEmpty() || it.getName().equals(validatorAction.getName())
      ).map(Arg::getText).toArray()
    );
    add(field.getProperty(), message);
  }
