...
public class DynaActionFormClass extends DefaultDynaClass {
  ...
  public DynaActionFormClass(FormBeanConfig config) {
    super(config.getName(), config.getFormProperties().stream().map(it -> new DynaProperty(it.getName(), it.getType())).toArray(DynaProperty[]::new));
    this.config = config;
    if (!config.getDynamic()) throw new IllegalArgumentException(String.format(
      "Failed to create a dynabean instance as the type [%s] of the form bean config [%s] is not a DynaBean class.",
      config.getFormClass().getName(), getName()
    ));
  }
  ...
  public DynaBean newInstance() {
    ...
    if (!DynaBean.class.isAssignableFrom(type)) throw new IllegalStateException(String.format(
      "The type [%s] of the form [%s] does not inherit DynaBean.", type.getName(), getName()
    ));
    ...
    return (DynaBean) createInstanceOf(
      type, List.of(FormBeanConfig.class), List.of(config), initialProps
    );
  }
}
