...
public class DynaActionFormClass implements DynaClass {
  ...
  public DynaActionFormClass(FormBeanConfig config) {
    ...
    for (var prop : config.getFormProperties()) {
      dynaPropertiesByName.put(prop.getName(), prop.getProperty());
    }
  ...
  public DynaBean newInstance() {
    ...
    if (!type.isAssignableFrom(DynaBean.class)) throw new IllegalStateException(String.format(
    ...
    return (DynaBean) createInstanceOf(
      config.getFormClass(), List.of(FormBeanConfig.class), List.of(this), initialProps
    );
  }
  ...
  private final Map<String, DynaProperty<?>> dynaPropertiesByName = new HashMap<>();
}
