...
public class ActionMessage implements Serializable {
  ...
  public String getText(@Nullable String bundle) {
    var resources = ModuleUtils.getCurrent().getMessageResources(bundle);
  ...
  public String getText(MessageResources resources) {
    ...
    var message = resources.getMessage(getKey(), getValues());
    if (message == null) throw new IllegalArgumentException(format(
      "The message associated with the key [%s] is not defined in the message resource [%s].",
      getKey(), resources.getConfig()
    ));
    return message;
  }
}
