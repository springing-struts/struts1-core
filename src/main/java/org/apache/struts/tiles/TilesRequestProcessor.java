...
public class TilesRequestProcessor extends RequestProcessor {
  ...
  protected boolean processTilesDefinition(
    ...
    var tilesDefinition = getModuleConfig().getTilesDefinitions().getTilesDefinitionByName(definitionName);
    if (tilesDefinition == null) {
      return false;
    }
    getActionContext().setTilesDefinition(tilesDefinition);
    doForward(tilesDefinition.getPath(), request, response);
    return true;
  }
}
