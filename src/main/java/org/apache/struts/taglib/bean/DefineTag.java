...
import static springing.util.ServletRequestUtils.convertValue;
...
public class DefineTag extends SetSupport implements JspVariableAware {
  ...
  public int doEndTag() throws JspException {
    var value = isValueAssigned ? assignedValue
              : hasText(ref.name) ? getValue(pageContext)
              : getBodyContent().getString();
    this.value = type == null ? value : convertValue(value, type);
    return super.doEndTag();
  }

  /**
   * Specifies the attribute name of the bean whose property is accessed to
   * define a new page scope attribute (if `property` is also specified) or the
   * attribute name of the bean that is duplicated with the new reference
   * created by this tag (if `property` is not also specified). This attribute
   * is required unless you specify a `value` attribute or nested body content.
   */
   ...
   * by `name`. This value may be a simple, indexed, or nested property
   * reference expression. If not specified, the bean identified by `name` is
   * given a new reference identified by `id`.
   */
