...
public interface ServletRequest extends jakarta.servlet.ServletRequest {
  ...
  static ServletRequest toJavaxNamespace(jakarta.servlet.ServletRequest orig) {
    if (orig instanceof ServletRequest inJavaxNamespace) {
      return inJavaxNamespace;
    }
    if (orig instanceof jakarta.servlet.http.HttpServletRequest httpServletRequest) {
      return HttpServletRequest.toJavaxNamespace(httpServletRequest);
    }
    return new Wrapper(orig);
  }

  class Wrapper implements ServletRequest {
    public Wrapper(jakarta.servlet.ServletRequest request) {
      orig = request;
    }
