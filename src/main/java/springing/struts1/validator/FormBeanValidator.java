...
public class FormBeanValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    var validators = form.getFormSet().getValidatorResources().getValidatorActions();
    for (var field : form.getFields()) {
      for (var depend : field.getDependencyList()) {
        var validator = validators.get(depend);
        if (validator == null) continue;
        var isValid = validator.test(target, field, actionErrors);
        if (!isValid) {
          for (var actionError : actionErrors.get()) {
            errors.rejectValue(
              field.getProperty(),
              actionError.getKey(),
              actionError.getValues(),
              ""
            );
            break;
          }
