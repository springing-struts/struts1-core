...
public class FormBeanValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    ...
    for (var field : form.getFields()) {
      for (var validatorName : field.getDependencyList()) {
        var validator = validators.get(validatorName);
        if (validator == null) {
          continue;
        }
        var validationErrors = new ActionErrors();
        var isValid = validator.test(target, field, validationErrors);
        if (isValid) {
          continue;
        }
        actionErrors.add(validationErrors);
        for (var error : validationErrors.get()) {
          errors.rejectValue(
            field.getProperty(),
            error.getKey(),
            error.getValues(),
