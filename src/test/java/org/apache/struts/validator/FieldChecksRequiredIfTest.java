...
public class FieldChecksRequiredIfTest {
  ...
  @Mock
  ValidatorAction mockValidatorAction;
  ...
  void prepareFixtures(
    Map<String, Object> fieldVars,
    Map<String, Object> formFieldValues
  ) {
    Map<String, Var> vars = fieldVars
      .entrySet()
      .stream()
      .collect(
        Collectors.toMap(Map.Entry::getKey, it ->
          new Var(it.getKey(), Objects.toString(it.getValue(), ""), null, null)
        )
      );
    ...
    vars.forEach((k, v) -> {
      ...
      when(mockField.getRequiredVarValue(eq(k), any())).thenReturn(
        v.getValue()
      );
    });
  ...
  @Test
  void itDoValidationWhenAllConditionsAreMetInTheAndJoinMode() {
  ...
  @Test
  void itDoValidationWhenAnyConditionAreMetInTheOrJoinMode() {
    Map<String, Object> fieldVars = Map.of(
      "fieldJoin",
      "OR",
      "field[0]",
  ...
  @Test
  void itThrowsErrorIfInvalidJoinTypeOrTestTypeWasAssigned() {
    prepareFixtures(
      Map.of(
        "fieldJoin",
        "INVALID_JOIN_TYPE",
    ...
    try {
      ...
      assertThat(e.getMessage()).contains(
        "Unknown requiredIf validation join type"
      );
    }
    ...
    try {
      ...
      assertThat(e.getMessage()).contains(
        "Unknown requiredIf validation test type"
      );
    }
  }
}
