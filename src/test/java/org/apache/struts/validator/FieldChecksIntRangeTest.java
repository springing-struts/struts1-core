...
public class FieldChecksIntRangeTest {
  ...
  void prepareFixture(
    ...
    when(mockField.getVarValueAsLong("min"))
      .thenReturn(min == null ? null : Long.parseLong(min));
    when(mockField.getVarValueAsLong("max"))
      .thenReturn(max == null ? null : Long.parseLong(max));
  ...
  void shouldPass() {
    assertThat(
      validateIntRange(mockBean, mockAction, mockField, mockErrors, mockValidator, mockRequest)
    ).isTrue();
